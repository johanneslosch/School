...
// Erweiterung 3:
// Erzeuge Methoden für die Addition und die Subtraktion von Objekten der Klas-
// se Polynom, die als Ergebnis wieder ein Polynom zurückgeben. Erzeuge für +,
// - und * passende Operatorüberladungen für Objekte der Klasse Polynom.  
...
{ public:
    ...
    Polynom Add(Polynom);       // Methode zur Addition
    Polynom Sub(Polynom);       // Methode zur Subtraktion
  private:
...
{ double *koeff = new double[grad+1],nwt; char zuf;
  ...
  cout << "\nf+f'(x) = "; P.Add(P.Abl()).Show();
  cout << "\nf-f'(x) = "; P.Sub(P.Abl()).Show();
  cout << "\nf'-f(x) = "; P.Abl().Sub(P).Show();
  cout << "\nGrad f  = " << P.Grad();
  cout << "\nGrad f' = " << P.Abl().Grad();
...
Polynom Polynom::Add(Polynom S)       // Methode zur Addition
{ Polynom E;
  for(int i=0;i<=grad;i++)
    E.Koeff[i]=Koeff[i]+S.Koeff[i];
  return E;  
}
Polynom Polynom::Sub(Polynom S)       // Methode zur Subtraktion
{ Polynom E;
  for(int i=0;i<=grad;i++)
    E.Koeff[i]=Koeff[i]-S.Koeff[i];
  return E;  
