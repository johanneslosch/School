...
bool ReadGeschlecht();
string name[50],vorname[50]; int geb[50]; bool frau[50];   
int main()
{ string nam,angaben; int h,ant,i=0,j; bool hb,sort;   
  while((i<50)&&(nam[0]!='#'))
  { cout << "Geben Sie Angaben zur Person Nr."<<i+1<<" ein!\n"
         << "Zum Eingabeabschluß geben Sie als Familiennnamen '#' ein!\n";
  ...
  while(true);
  { clrscr();
    cout << "Sie können die Angaben zur Person jetzt sortiert ausgeben.\n"
    ...
    { case 1: if(i>0) do
            { sort=false;
              { for(j=0;j+1<i;j++)
                { if(name[j]<name[j+1])
                  { nam=name[j];name[j]=name[j+1];name[j+1]=nam;
                    nam=vorname[j];vorname[j]=vorname[j+1];vorname[j+1]=nam;
                    h=geb[j];geb[j]=geb[j+1];geb[j+1]=h;
                    hb=frau[j];frau[j]=frau[j+1];frau[j+1]=h;
                    sort=true;                    
                  }
                }    
              }
